...
\\ \label{eqns:motion-vector}
    & \dot{m}\vec{v} + m \dot{\vec{v}} = T(\dot{m}) \vec{e}_a + {L_p}(\vec v - \vec u, \alpha) \vec{e}_p  + {L_r}(\vec v - \vec u,\beta) \vec{e}_r + \vec w,
% \implies
% \\
...
For the lift force $L_r$, we assume it is enough to balance the external vertical, or radial, forces. That is, by taking dot product of $\vec e_r$ on either side of \eqref{eqns:motion-vector}, and using $\vec w \cdot \vec e_r = -  {L_r}(\vec v - \vec u,\beta)$ we obtain:
\begin{equation}
    m \dot{\vec{v}} \cdot \vec e_r = - \dot{m}\vec{v} \cdot \vec e_r.
\end{equation}

nor for the lift $L_r$ to simplify the equations of motion. 

Equation \eqref{eqns:motion-vector} can be used to update the velocity vector $\vec v$ in time, which we show in more detail in the next section. 

Turning the plane also changes its orientation, which as a consequence changes the basis vectors $\vec e_a$ and $\vec e_p$. To capture this we can use:



To simplify \eqref{eqns:motion-vector} we need to write all expressions in terms of the local coordinate basis $\vec e_a, \vec e_p, \vec e_r$. First we assume that the external forces can be decomposed in the form:
\[
\vec w = w_a \vec e_a + w_p \vec e_p + w_r \vec e_r.
\]

Next we use $\vec v = v_a \vec e_a + v_p \vec e_p$ to rewrite the expression:
\begin{equation} \label{eqn:mass-momentum}
    \dot{m}\vec{v} + m \dot{\vec{v}} = \dot v_a \vec e_a + \dot v_p \vec e_p + \dot v_r \vec e_r,
    % \dot{m}v \vec{e}_v + m \dot v \vec{e}_v + m v \dot{\vec{e}}_v.
\end{equation}
where we also used 
\[
\dot{\vec{e}}_a =  (\dot{\vec{e}}_a \cdot \vec{e}_p)\vec{e}_p + (\dot{\vec{e}}_a \cdot \vec{e}_r)\vec{e}_r \quad 
\dot{\vec{e}}_p =  (\dot{\vec{e}}_p \cdot \vec{e}_a)\vec{e}_a + (\dot{\vec{e}}_p \cdot \vec{e}_r)\vec{e}_r.
\] 
