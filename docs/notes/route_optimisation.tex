...
To simplify \eqref{eqns:motion-vector} we need to write all expressions in terms of the local coordinate basis $\vec e_v, \vec e_p, \vec e_r$.

To find $\dot{\vec{e}_v}(t)=  (\dot{\vec{e}_v}(t) \cdot \vec{e}_r)\vec{e}_r +(\dot{\vec{e}_v}(t) \cdot \vec{e}_p)\vec{e}_p+ (\dot{\vec{e}_v}(t) \cdot \vec{e}_r)\vec{e}_r$ then we use that 

\begin{equation}
  \frac{d}{dt}(\vec{e}_v \cdot \vec{e}_v)=0\quad \text{and} \quad \frac{d}{dt} (\vec{e}_v \cdot \vec{e}_r)=0  \quad \text{and} \quad \frac{d}{dt}(\vec{e}_v \cdot \vec{e}_p) = 0
\end{equation}
...
\begin{align} \label{eqn:thrust-momentum}
    &  \dot{m}(t)v(t) +m(t)\dot v(t)= w_h(r,v,t) - T(\dot{m},v(t))v(t) 
    \\
    \label{eqn:turn-momentum}
    & - m(t) v(t)(\dot{\vec{e}_p}(t) \cdot \vec{e}_v) = w_n(r,v,t) + L_c(\alpha(t),v(t)
    \\
